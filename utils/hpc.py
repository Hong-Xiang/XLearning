...
def prepare_jsons(def_c, replica=1, batch_size=None, input_shape=None, lr=None, filters=None, depths=None, blocks=None, cores=None, is_bn=None,
                  s8=1, s4=1, s2=1, suf=10, saf=1000):
    net_c = dict(def_c)
    ds_c2x = {
    ...
    for i in range(replica):
        path = "rs%df%dd%db%dc%dl%0.1e.%d" % (
            net_c['batch_size'], net_c['filters'], net_c['depths'], net_c['blocks'], net_c['cores'], net_c['lrs'], i)
        if net_c['is_bn']:
            path += 'bn'
        path = os.path.abspath(path)
        os.mkdir(path)
        with open(os.path.join(path, 'sino2_shep8x.json'), 'w') as fout:
            json.dump(ds_c8x, fout, indent=4, separators=[
                      ',', ': '], sort_keys=True)
        with open(os.path.join(path, 'sino2_shep4x.json'), 'w') as fout:
            json.dump(ds_c4x, fout, indent=4, separators=[
                      ',', ': '], sort_keys=True)
        with open(os.path.join(path, 'sino2_shep2x.json'), 'w') as fout:
            json.dump(ds_c2x, fout, indent=4, separators=[
                      ',', ': '], sort_keys=True)
        with open(os.path.join(path, 'srsino8v2.json'), 'w') as fout:
            json.dump(net_c, fout, indent=4, separators=[
                      ',', ': '], sort_keys=True)
        xlearn_path = os.environ['PATH_XLEARN']
        with open(os.path.join(path, 'work.sh'), 'w') as fout:
            print("python " + xlearn_path + r"/scripts/main.py train_sino8v2 --total_step 1000 " +
                  "--step8 %d --step4 %d --step2 %d --sumf %d --savef %d" % (s8, s4, s2, suf, saf), file=fout)
        shutil.copy(xlearn_path + r"/template/k80.slurm",
                    os.path.join(path, 'k80.slurm'))
        shutil.copy(xlearn_path + r"/template/p100.slurm",
                    os.path.join(path, 'p100.slurm'))
